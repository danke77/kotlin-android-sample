...
class CharacterParser private constructor() {
    ...
    fun convert(str: String): String? {
        ...
        if (ascii > 0 && ascii < 160) {
            ...
            for (i in CharacterParser.Companion.PY_VALUE.size - 1 downTo 0) {
                if (CharacterParser.Companion.PY_VALUE[i] <= ascii) {
                    result = CharacterParser.Companion.PY_STR[i]
                    break
                }
            }
        }
    ...
    fun getSelling(chs: String): String? {
        ...
        for (i in 0..chs.length - 1) {
            ...
            if (key.toByteArray().size >= 2) {
                value = convert(key)
                if (value == null) {
                    value = CharacterParser.Companion.UNKNOWN_TAG
                }
            } else {
                value = key
            }
